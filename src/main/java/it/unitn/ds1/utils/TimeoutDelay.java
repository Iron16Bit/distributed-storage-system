...
public class TimeoutDelay {
    ...
    public static final int CLIENT_GET_DELAY = (int)(BASE_DELAY * 4.5);     // Includes quorum read + response
    public static final int CLIENT_UPDATE_DELAY = (int)(BASE_DELAY * 4.75);  // Includes read-before-write + quorum write
    
    // Node lifecycle operation delays (complete operations)
    public static final int JOIN_DELAY = (int)(BASE_DELAY * 6.2);           // Complete join process with data transfer
    public static final int LEAVE_DELAY = (int)(BASE_DELAY * 3.2);          // Complete leave process with data redistribution
    public static final int CRASH_DELAY = BASE_DELAY;                       // Immediate crash
    public static final int RECOVERY_DELAY = (int)(BASE_DELAY * 4.2);       // Complete recovery with data sync
